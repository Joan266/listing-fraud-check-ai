...
def validate_address_and_get_place_details(address: str) -> dict:
    """
    Validates an address and gets details from Google Places API.
    This is the core of the CRITICAL CHECKPOINT.
    """
    if not gmaps:
    ...
    try:
        ...
        if not place_id:
            report["error"] = "Geocoded address has no Place ID, likely a residential address with no business listing."
            return report

        # Step 2: Use Place ID to get rich details from Places API
        report["place_id"] = place_id
        fields = ['type', 'rating', 'user_ratings_total']
        ...
        if place_details.get('result'):
            ...
            report["place_user_ratings_total"] = result.get('user_ratings_total')

    except googlemaps.exceptions.ApiError as e:
        logger.error(f"Google Maps API error: {e}")
        report["error"] = f"Google Maps API error: {e}"
    except Exception as e:
