...
import { Search, Link as LinkIcon, ShieldCheck, Type } from 'lucide-react';
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="mb-6">
          ...
          {inputMode === 'text' ? (
            ...
            <input
              ...
              className={`w-full p-4 border rounded-lg focus:ring-2 focus:ring-yellow-400 focus:border-transparent transition-all ${theme === 'dark'
                ? 'bg-gray-800 border-gray-700 text-white placeholder-gray-400'
                : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500'
                }`}
            />
          ...
          >
            {isExtracting ? (
              <>
                ...
                <span>{inputMode === 'url' ? 'Scraping & Extracting...' : 'Loading...'}</span>
              </>
            ) : (
              <>
                <Search size={20} />
