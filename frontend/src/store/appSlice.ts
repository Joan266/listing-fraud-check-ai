...
import { AppState, Analysis, ExtractedData, FinalReport, ChatMessage } from '../types';
...
export const submitAnalysisAsync = createAsyncThunk(
  ...
  async (extractedData: ExtractedData, { getState, dispatch }) => {
    ...
    const response = await apiClient.submitAnalysis(extractedData, sessionId, state.app.currentAnalysis?.id || undefined);
...
export const updateAnalysisAsync = createAsyncThunk(
  ...
  async ({ checkId, extractedData }: { checkId: string; extractedData: ExtractedData }, { getState, dispatch }) => {
    ...
    const response = await apiClient.submitAnalysis(extractedData, sessionId, checkId);
...
const appSlice = createSlice({
      ...
      if (analysis) {
        ...
        if (analysis.finalReport) {
          state.chatMessages = [{
            id: uuidv4(),
            type: 'assistant',
            content: analysis.finalReport.chat_explanation,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(submitAnalysisAsync.pending, (state,action) => {

        state.loading = true;

        if (state.currentAnalysis) {

          state.currentAnalysis.extractedData = action.meta.arg;
      ...
      .addCase(pollAnalysisStatus.fulfilled, (state, action) => {
        if (state.currentAnalysis) {
          ...
          const initialMessage: ChatMessage = {
            id: uuidv4(),
            type: 'assistant',
            content: action.payload.chat_explanation,
            timestamp: new Date().toISOString()
          };
          state.chatMessages = [initialMessage];
